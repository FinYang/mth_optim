...
Before the algorithm enters the employed bee phase, the total number of food sources (the solution set) is to be determined and denoted as $SN$ (swarm size). We will discuss the impact of swarm size in more detail in the parameter section. @akay2009parameter argue the robustness of the algorithm is not affected by different swarm size in general.
...
2. In Figure \@ref(fig:p6), \@ref(fig:p7), and \@ref(fig:p8), for example, there are some points on the edge of a clear cluster is identified to be in another cluster. This is because the allocation scheme we are using is closest allocation: allocation depends on the distance of the point to the centroids instead of pattens. We could improve the algorithm if take the pattern into consideration in allocation.
