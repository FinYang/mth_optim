...
The parameters we choose to evaluate and select are The limit (the number of iteartion that each food source can stay the same before being abandoned), the maximum cycle number (MCN) and the number of iteration with no improvment.  The number of food sources $SN$, is well studyed in @akay2009parameter.They argue that the algorithm is robust with different value of $SN$ in general.
...
We then report the different values we tried in Table 2-6 in the Appendix. 
