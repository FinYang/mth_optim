...
We implement a coordinate descent approach where we try different values of one parameter and keep other parameters fixed at the same time. We firstly keep the limit fixed as by @akay2009parameter a value of $SN\times D$ is suited to data with general form. We also keep MCN as 1000 since from the results above no data set rearches MCN of 1,000, but quite a few data sets stops when the number of unchanged solution rearches 200. We acknowdge that there might be correlation issue where the effects of parameters are not independent with each other especially when MCN and the max number of unchange control the stopping of the algorithm together.

We then report the different values we tried. We start with MCN of 1000 and limit of $SN\times D$, and increase MNU see if additional iteration can improve the optimization. The results for the two data sets are reported in Table \@ref(fig:sq1) and Table \@ref(fig:te1). If we simply look at the final objective value, then increasing MNU would always increase the performence. Because the MNC and MNU controls the stopping condition, it would be ideal if we can find a balance that about half of the time algorithm stops because of MNC and the other half because of MNU, not wasting any unneccary runs. For data `4_tetra` the algorithm always stops because of the MNU and for `3_square2` only when the MNU is larger enough to 600 it would reach MCN, so we would aim to decrease MNC instead of MNU considering the time it costs to run more iterations.

We keep MNU at 200 and try different value for MNC. The results are reported in Table \@ref(fig:sq2) ann \@ref(fig:te2). MCN of 500 is optimal for `3_square2` and 600 for `4_tetra`, so we take the average of 550, but also keep in mind that the number of clusters for these two data set are reletively small. Then we try different value for the limit in Table \@ref(fig:sq3) ann \@ref(fig:te3). The good values of limit range between 30 and 50, we take the average of 40. Note the number of iteration reaches the maximum number multiple times, so we decided to increase MCN to 700 to accomendate high dimensional data and data with more clusters.
...
We evaluate the performence of our new parameters on the training set`
